...
import { toUserFacingMemberEditError } from "@/lib/user-facing-errors";
...
type MemberEditFormProps = {
  ...
  initialExecTitle: ExecTitle | null;
};
...
export default function MemberEditForm({
  ...
  // The role-assignment flow stamps a literal "-" as a placeholder phone number -
  // showing that back as a pre-filled value would just invite someone to save it
  // again unchanged.
  ...
  const [execTitle, setExecTitle] = useState<ExecTitle | "">(initialExecTitle ?? "");
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (error) {
      setLoading(false);
      setSaveError(toUserFacingMemberEditError(error));
      return;
    }
    ...
    if (membershipError) {
      setSaveError(toUserFacingMemberEditError(membershipError));
      return;
    }

    router.push(`/members/${member.id}?updated=1`);
    router.refresh();
  }

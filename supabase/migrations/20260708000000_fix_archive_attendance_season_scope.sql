-- Keep archive balances and attendance auto-flagging scoped to the explicit season.
...
begin
  ...
  -- Block concurrent finance writes while computing the frozen carryover.
...
begin
  ...
  where (
      (cs.type = 'exec meeting' and sm.exec_title is not null)
      or (cs.type <> 'exec meeting' and 'dancer' = any (m.roles))
    )
    and not exists (
      select 1
...
declare
  v_session_type text;
  v_session_season text;
begin
  if public.get_my_exec_title() not in ('captain', 'team_manager') then
    raise exception 'Not authorized to close practice sessions';
  end if;

  select ps.type, ps.season
  into v_session_type, v_session_season
  ...
  join public.members m
    on m.id = sm.member_id
  where sm.season = v_session_season
    and sm.status = 'active'
    and (
      (v_session_type = 'exec meeting' and sm.exec_title is not null)
      or (v_session_type <> 'exec meeting' and 'dancer' = any (m.roles))
    )
    and exists (
      select 1
      from public.practice_sessions ps

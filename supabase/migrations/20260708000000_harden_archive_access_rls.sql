-- Harden season archive access and read-only enforcement.

-- Exec access is season-scoped. Legacy members.roles alone must not keep
-- revoked/alumni execs authorized after archive.
...
as $$
  select exists (
    ...
    where m.email = lower(auth.jwt() ->> 'email')
      and 'exec' = any (coalesce(m.roles, array[]::text[]))
      and sm.exec_title in ('captain', 'team_manager', 'finance')
  );
$$;

-- Normal authenticated clients may create future season rows, but they cannot
-- create archived rows, mutate archived rows, or delete season registry rows.
drop policy if exists "Reject archived season inserts" on public.seasons;
create policy "Reject archived season inserts"
on public.seasons
as restrictive
for insert
to authenticated
...
drop policy if exists "Reject archived season updates" on public.seasons;
create policy "Reject archived season updates"
on public.seasons
as restrictive
for update
to authenticated
...
drop policy if exists "Reject season deletes" on public.seasons;
create policy "Reject season deletes"
on public.seasons
as restrictive
for delete
to authenticated
...
-- Replaces the prior version to require at least one next-season admin.
...
begin
  ...
  if not exists (
    select 1
    from jsonb_array_elements(p_members) elem
    where elem->>'status' <> 'alumni'
      and elem->>'next_exec_title' in ('captain', 'team_manager')
  ) then
    raise exception 'At least one Captain or Team Manager is required for the next season';
  end if;
...
-- Reimbursements inherit archive status through competitions.
drop policy if exists "Reject reimbursement inserts for archived competitions" on public.reimbursements;
create policy "Reject reimbursement inserts for archived competitions"
...
with check (
  ...
  or not public.is_competition_season_archived(competition_id)
);

drop policy if exists "Reject reimbursement updates for archived competitions" on public.reimbursements;
create policy "Reject reimbursement updates for archived competitions"
on public.reimbursements
as restrictive
for update
to authenticated
...
create or replace function public.is_registration_packet_object_archived(p_name text)
returns boolean
language plpgsql
stable
security definer
...
begin
  begin
    v_competition_id := nullif((storage.foldername(p_name))[1], '')::uuid;
  exception
  ...
  return public.is_competition_season_archived(v_competition_id);
end;
$$;

grant execute on function public.is_registration_packet_object_archived(text) to authenticated;

drop policy if exists "Reject registration packet inserts for archived competitions" on storage.objects;
create policy "Reject registration packet inserts for archived competitions"
on storage.objects
as restrictive
for insert
to authenticated
with check (
  ...
  or not public.is_registration_packet_object_archived(name)
);

drop policy if exists "Reject registration packet updates for archived competitions" on storage.objects;
create policy "Reject registration packet updates for archived competitions"
on storage.objects
as restrictive
for update
to authenticated
...
drop policy if exists "Reject registration packet deletes for archived competitions" on storage.objects;
create policy "Reject registration packet deletes for archived competitions"
on storage.objects
as restrictive
for delete
to authenticated

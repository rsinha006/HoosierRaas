-- save_competition_packet_data inserted new deadlines and updated matched existing
-- ones, but never deleted a deadline that the reviewer removed from the form —
-- fees and contacts already do a full delete-then-reinsert, but deadlines can't use
-- that approach since their status/completed_at needs to survive a re-save. Track
-- which existing deadline ids were kept (matched or newly inserted) and delete
-- anything for this competition that wasn't.

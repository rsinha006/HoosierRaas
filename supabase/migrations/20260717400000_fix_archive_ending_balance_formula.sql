-- archive_season computed the carryover balance from ALL income and ALL approved
-- expenses — including IUFB income/expenses, which must never mix into the general
-- pool (the same bug already fixed on the finance dashboard) — and never subtracted
-- paid reimbursements at all. That means every season archived so far may have
-- carried over a wrong balance into the next season. Match the same general-pool
-- formula the dashboard and computeSeasonEndingBalance() use:
-- general pool income - general pool approved expenses - paid reimbursements.
...
declare
  ...
  v_paid_reimbursements numeric(12, 2);
...
begin
  ...
  -- General pool only — IUFB income never counts toward the team's own balance.
  ...
  -- General pool only — expenses funded by an IUFB line item are excluded.
  select coalesce(sum(amount), 0)
  ...
  where status = 'approved'
    ...
    and iufb_line_item_id is null;

  -- Paid reimbursements always come out of the general pool too.
  select coalesce(sum(amount), 0)
  into v_paid_reimbursements
  from public.reimbursements
  where status = 'paid'
    and payment_timestamp >= v_expense_start
    and payment_timestamp <= v_expense_end;

  v_balance := v_total_income - v_approved_expenses - v_paid_reimbursements;

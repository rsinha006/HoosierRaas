-- Reimbursements now carry an explicit season. Archive carryover should count
-- paid reimbursements from the season they belong to, not from the date they
-- happened to be marked paid. Otherwise late payments after the season closes
-- are dropped from the archived season and can be charged to the next one.
...
begin
  ...
  if p_members is null or jsonb_typeof(p_members) <> 'array' or jsonb_array_length(p_members) = 0 then
    raise exception 'This season has no roster members, so there is nothing to carry over. Add at least one member to the roster before archiving.';
  end if;

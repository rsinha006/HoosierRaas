-- AI-extracted packet data lived only in sessionStorage in the tab that ran the
-- extraction - opening the review screen in a second tab, or a browser crash,
-- lost it and bounced the reviewer back to the competition page with nothing to
-- show for the extraction that just ran. sessionStorage stays as the fast path
-- (no round trip needed for the common case, same tab that just extracted), but
-- the merged draft is now also written to the competition row so any session
-- with access to this competition can recover it.
--
-- Cleared on confirm (below, as part of save_competition_packet_data) and on
-- cancel (app-side). A draft that's simply abandoned - navigated away from
-- without cancelling - is overwritten the next time extraction runs, or left as
-- an unused column value with the same RLS as the rest of the row; nothing here
-- is reachable by anyone who isn't already exec.
-- Run this in the Supabase SQL Editor before merging the app change.

alter table public.competitions
  add column if not exists pending_packet_extraction jsonb;
...
begin
  ...
  set
    ...
    pending_packet_extraction = null
  ...
  loop
    ...
    if v_deadline.id is null then
      ...
      v_keep_ids := array_append(v_keep_ids, v_deadline.id);
    end if;
  end loop;
